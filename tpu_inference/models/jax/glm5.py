...
# GLM-5.2 (GlmMoeDsa, 744B-A40B) config. Source of truth: HF zai-org/GLM-5.2/config.json and
# the vLLM GPU reference vllm/model_executor/models/deepseek_v2.py (GlmMoeDsaForCausalLM is the
# DeepSeek-V3.2 / is_v32 path). Adapted from deepseek_v3.py; these module globals are the single
# place the model dims are set (the skeleton does not read most dims from hf_config -> config.json
# must use matching hidden_size / vocab_size / num_hidden_layers / kv_lora_rank).
# NOTE: the DSA lightning indexer (index_topk=2048, index_n_heads=32, index_head_dim=128) is
# DEFERRED for Step 1 -- it is a numerical no-op at <=2048 tokens and is not wired in this skeleton.
num_local_experts: int = int(os.environ.get("GLM_NUM_EXPERTS", "256"))  # override for reduced-expert MoE parity tests (real GLM-5.2 = 256)
vocab_size: int = 154880
hidden_size: int = 6144
num_attention_heads: int = 64
num_key_value_heads: int = 64
ffw_intermediate_size: int = 12288  # dense-layer MLP (HF intermediate_size)
moe_intermediate_size: int = 2048
num_experts_per_token: int = 8
n_group: int = 1  # GLM-5.2 disables node-limited group routing (n_group=1, topk_group=1)
interleave_moe_layer_step: int = 1  # GLM-5.2 moe_layer_freq=1
hidden_act: str = "silu"
rms_norm_eps: float = 1e-05
routed_scaling_factor: float = 2.5
first_k_dense_replace: int = 3  # first 3 layers dense, rest MoE

num_shared_experts = 1
rope_theta = 8000000  # GLM-5.2 rope_theta (8e6)
# GLM-5.2 uses standard RoPE (rope_type "default", no YaRN). This skeleton's RoPE is YaRN-based;
# we collapse YaRN to standard by setting factor=1.0 (=> yarn_mscale=1.0 and inv_freq reduces to
# the standard schedule); interleaved layout is retained by DeepseekScalingRotaryEmbedding.
# original_max_position_embeddings must be >= the max test sequence length.
rope_scaling = {
    "beta_fast": 32,
    "beta_slow": 1,
    "factor": 1.0,
...
class MLAEinsum(JaxEinsum):
    ...
    def load_weights(self, weights):
        ...
        # BF16 (unquantized) path: GLM-5.2 real checkpoints are pure BF16. vLLM may
        # still hand a non-None quant_config object even when nothing is quantized,
        # so detect the unquantized case by the ABSENCE of the FP8 weight_scale_inv
        # param (the FP8 path requires it). After the per-param weight_loader runs
        # above, self.weight.value is already transposed to (kv_lora_rank,
        # N*(qk_nope+v_head)) == (A, N*(qk_nope_head_dim+v_head_dim)) -- same [in, out]
        # layout the FP8 path produces post-dequant. Reshape, split into k/v, and
        # overwrite the pre-created (random-init) k_up_proj/v_up_proj kernels in place.
        if not hasattr(self, "weight_scale_inv"):
            mla_layer = self.mla_layer
            A, N, qk_nope_head_dim, v_head_dim = (
                mla_layer.kv_lora_rank, mla_layer.N,
                mla_layer.qk_nope_head_dim, mla_layer.v_head_dim)
            # self.weight.value is already a TPU-sharded array (loaded onto the
            # device mesh by the per-param weight_loader), so do the reshape/split
            # on it directly -- do NOT force a CPU mesh (that caused a TPU-vs-CPU
            # "incompatible devices" error).
            fused_weight = self.weight.value
            if fused_weight.shape != (A, N * (qk_nope_head_dim + v_head_dim)):
                raise ValueError(
                    f"Unexpected kv_b_proj weight shape: {fused_weight.shape}, "
                    f"expected {(A, N * (qk_nope_head_dim + v_head_dim))=}")
            fused_weight = fused_weight.reshape(
                A, N, qk_nope_head_dim + v_head_dim)
            k_ANH, v_ANH = jnp.split(fused_weight, [qk_nope_head_dim], axis=-1)
            mla_layer.k_up_proj.weight.value = shard_put(
                k_ANH, mla_layer.anh_sharding)
            mla_layer.v_up_proj.weight.value = shard_put(
                v_ANH, mla_layer.anh_sharding)
            delattr(self, 'weight')
            return
...
class DeepseekV3MLA(DeepseekV3BaseAttention):
    ...
    def __post_init__(self, rngs: nnx.Rngs):
        ...
        # GLM-5.2 Step-1 (Trap #2): k_up_proj/v_up_proj are normally created inside
        # MLAEinsum.load_weights (the FP8 "absorb" path). For random/dummy weights -- and the
        # later BF16 real-weight path -- that FP8 path does not run, so compute_q_projection
        # (-> self.k_up_proj) and process_output (-> self.v_up_proj) would hit AttributeError.
        # Create them here so the MLA forward has its params. For real FP8 weights,
        # MLAEinsum.load_weights overwrites these two via setattr after dequant/split.
...
class DeepSeekV3(JaxModule):
    ...
    def __init__(self,
        ...
        # GLM-5.2 has 78 decoder layers and (unlike DeepSeek-V3) no trailing MTP
        # layer, so we build exactly hf_config.num_hidden_layers layers. For the
        # Step-1 smoke test this is set to 1 in config.json.
...
class GlmMoeDsaForCausalLM(JaxModule, LoadableWithIterator):
    """GLM-5.2 (GlmMoeDsa) for CausalLM, native Flax NNX.

    Adapted from deepseek_v3.py's DeepseekV3ForCausalLM: GLM-5.2 is the DeepSeek-V3.2
    topology (MLA + sigmoid MoE + DSA). DSA indexer is deferred for Step 1 (no-op at
    <=2048 tokens). Correctness reference: vLLM GPU deepseek_v2.py (GlmMoeDsaForCausalLM).
    """
    ...
    def load_weights(self, weights: Iterable) -> set[str]:
        ...
        # GLM-5.2 has 78 decoder layers (no MTP layer, unlike DeepSeek-V3). For the
        # Step-1 smoke test num_hidden_layers may be set to 1, so we build only the
        # first `num_built` layers. The checkpoint, however, may contain weights for
        # all of its layers, so we must drop any checkpoint weight whose layer index
        # is >= num_built. We do this by filtering the weights iterable with an
        # integer-parsed layer index (NOT a substring match, which would mis-handle
        # e.g. "layers.1" matching "layers.10"), which is robust to any checkpoint
        # layer count without a hardcoded upper bound.
        #
        # We also drop the DSA "lightning indexer" params (self_attn.indexer.*) --
        # k_norm{.weight,.bias}, weights_proj.weight, wk.weight, wq_b.weight -- which
        # this skeleton does not implement; otherwise the auto-loader raises on them
        # (no matching module/param).
        num_built = len(self.model.layers)
        _layer_re = re.compile(r"(?:^|\.)layers\.(\d+)\.")

        def _filter_weights(weights_iter):
            for name, weight in weights_iter:
                m = _layer_re.search(name)
                if m is not None and int(m.group(1)) >= num_built:
                    continue
                if "indexer" in name:
                    continue
                yield name, weight
        ...
        loaded = loader.load_weights(_filter_weights(weights))
